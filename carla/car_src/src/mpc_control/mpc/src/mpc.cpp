...
void MPC::init(ros::NodeHandle &nh)
{
    ...
    nh.param("mpc/feed_forward", feed_forward, false);
    ...
    pos_cmd_pub_ = nh.advertise<ackermann_msgs::AckermannDrive>("cmd", 10);
    ...
    true_odom_pub = nh.advertise<nav_msgs::Odometry>("/true_odom", 10);

    debug_pose_pub = nh.advertise<visualization_msgs::Marker>("/debug_pose", 1);
    err_pub = nh.advertise<std_msgs::Float64>("/track_err", 10);

    cmd_timer_ = nh.createTimer(ros::Duration(0.01), &MPC::cmdCallback, this);
    // odom_sub_ = nh.subscribe("odom", 1, &MPC::odomCallback, this);
    status_sub_ = nh.subscribe("status", 1, &MPC::statusCallback, this);
    ...
    if (in_test)
    {
        ...
        receive_traj_ = false;
        init_pub = nh.advertise<geometry_msgs::PoseWithCovarianceStamped>("/initialpose", 10);
        trigger_sub_ = nh.subscribe("/move_base_simple/goal", 1, &MPC::triggerCallback, this);
        string o = test_traj;
        outfile.open (o.insert(16,"err_"), std::ofstream::out);
        outfile.clear();
        trajectory_analyzer_.setTraj(test_traj.c_str());
    }
    else
    {
        traj_sub_ = nh.subscribe("traj", 1, &MPC::trajCallback, this);
    }
}

void MPC::odomCallback(nav_msgs::OdometryConstPtr msg)
{
    ;
}

void MPC::trajCallback(const mpc::TrajectoryConstPtr msg)
{
    trajectory_analyzer_.setTraj(msg);
    receive_traj_ = true;
}

void MPC::triggerCallback(const geometry_msgs::PoseStamped msg)
{
    if (test_traj[16]=='m')
    {
        in_test = false;
        Eigen::Vector3d initp = trajectory_analyzer_.nextInitPointMinco();
        geometry_msgs::PoseWithCovarianceStamped initg;
        initg.header.frame_id = "map";
        initg.pose.pose.position.x = initp[0];
        initg.pose.pose.position.y = initp[1];
        initg.pose.pose.position.z = 0.1;
        Eigen::Matrix3d R;
        R<<cos(initp[2]),-sin(initp[2]),0,\
            sin(initp[2]),cos(initp[2]),0,\
            0,0,1;
        Eigen::Quaterniond q(R);
        initg.pose.pose.orientation.w = q.w();
        initg.pose.pose.orientation.x = q.x();
        initg.pose.pose.orientation.y = q.y();
        initg.pose.pose.orientation.z = q.z();
        init_pub.publish(initg);
        trajectory_analyzer_.beginNextTrajMinco();
        receive_traj_ = true;
    }
    else
    {
        Eigen::Vector3d initp = trajectory_analyzer_.nextInitPoint();
        geometry_msgs::PoseWithCovarianceStamped initg;
        initg.header.frame_id = "map";
        initg.pose.pose.position.x = initp[0];
        initg.pose.pose.position.y = initp[1];
        ...
        initg.pose.pose.orientation.z = q.z();
        init_pub.publish(initg);
        trajectory_analyzer_.beginNextTraj();
        receive_traj_ = true;
    }
    
}

void MPC::statusCallback(const carla_msgs::CarlaEgoVehicleStatusConstPtr msg)
{
    has_odom = true;
    now_state.x = msg->pose.position.x;
    now_state.y = msg->pose.position.y;
    ...
    now_state.v = msg->velocity;

    nav_msgs::Odometry o;
    o.pose.pose = msg->pose;
    o.pose.pose.position.z = 0.4;
    o.header = msg->header;
    true_odom_pub.publish(o);
}
...
void MPC::cmdCallback(const ros::TimerEvent &e)
{
    ...
    {
        drawFollowPath();
        vector<TrajPoint> P = trajectory_analyzer_.getRefWaypoints(T, dt);
        TrajPoint perr = trajectory_analyzer_.getErrWaystate();
        std_msgs::Float64 err;
        err.data = sqrt(pow(perr.x - now_state.x, 2) + pow(perr.y - now_state.y, 2));
        err_pub.publish(err);
        if (trajectory_analyzer_.at_goal)
        {
            cmd.speed = 0.0;
            cmd.acceleration = -8.0;
        ...
        {
            outfile<<perr.relative_time<<","<<err.data<<std::endl;
            for (int i=0; i<T; i++)
            {
                xref(0, i) = P[i].x;
                xref(1, i) = P[i].y;
            ...
            smooth_yaw();
            getCmd();
            if (trajectory_analyzer_.back_track)
                cmd.speed = -9999.9;
            else
                cmd.speed = 9999.9;
        }
    ...
    {
        // drawFollowPath();
        vector<TrajPoint> P = trajectory_analyzer_.getRefPoints(T, dt);
        TrajPoint perr = trajectory_analyzer_.getErrState();
        std_msgs::Float64 err;
        err.data = sqrt(pow(perr.x - now_state.x, 2) + pow(perr.y - now_state.y, 2));
        err_pub.publish(err);
        if (trajectory_analyzer_.at_goal)
    ...
    visualization_msgs::Marker debug_pose;
    debug_pose.header.stamp = ros::Time::now();
    debug_pose.header.frame_id = "map";
    debug_pose.type = visualization_msgs::Marker::SPHERE;
    debug_pose.pose.position.x = trajectory_analyzer_.debug_pose.x;
    debug_pose.pose.position.y = trajectory_analyzer_.debug_pose.y;
    debug_pose.pose.position.z = 0.5;
    debug_pose.pose.orientation.w = 1.0;
    debug_pose.scale.x = 0.1;
    debug_pose.scale.y = 0.1;
    debug_pose.scale.z = 1.0;
    debug_pose.color.a = 1.0;
    debug_pose.color.g = 1.0;
    debug_pose_pub.publish(debug_pose);
    pos_cmd_pub_.publish(cmd);
}
