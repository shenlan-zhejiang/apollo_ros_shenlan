...
*/
  ...
  //   tfs_.transform.translation.x = it.transform().translation().x();
  //   tfs_.transform.translation.y = it.transform().translation().y();
  //   tfs_.transform.translation.z = it.transform().translation().z();
  ...
  for (auto it : obj2.transforms()) {
    geometry_msgs::TransformStamped tfs_;
    tfs_.header.stamp = ros::Time::now(); //Time::now(); //(it.header().timestamp_sec());
    // tfs_.header.stamp = ros::Time(obj2.header().timestamp_sec());
    tfs_.header.seq = obj2.header().sequence_num();
    // std::cout << "ros::WallTime::now: " << ros::WallTime::now() << std::endl;
    tfs_.header.frame_id = "agent_0"; 
    tfs_.child_frame_id = "lidar";
    tfs_.transform.translation.x = 0; // it.transform().translation().x() - it.transform().translation().x();
    tfs_.transform.translation.y = 0; // it.transform().translation().y() - it.transform().translation().y();
    tfs_.transform.translation.z = 1.7; // it.transform().translation().z();
    tfs_.transform.rotation.x = 0; // it.transform().rotation().qx();
    tfs_.transform.rotation.y = 0; // it.transform().rotation().qy();
    tfs_.transform.rotation.z = 0.7071; // it.transform().rotation().qz();
    tfs_.transform.rotation.w = 0.7071; // it.transform().rotation().qw();
    ...
    // tfs_.transform.rotation.z = 0;
    // tfs_.transform.rotation.w = 1;
    tf_.transforms.emplace_back(tfs_);
  }
...
int main(int argc, char *argv[]) {
  ros::init(argc, argv, "tfsatic"); //节点名为study，随便取
  ros::NodeHandle n;
  struct para_t para;
  ...
  para.cap = 0;
  para.pub = n.advertise<tf2_msgs::TFMessage>("/tf_static", 1000);
  para.port = 8905;
  para.pfd = -1;
  para.index = -1;
