...
*/
  ...
  for (auto it : obj2.transforms()) {
    ...
    tfs_.header.frame_id = "car_0_pos"; 
    tfs_.child_frame_id = "agent_0";
    ...
    tfs_.transform.translation.z = 0; // it.transform().translation().z();
    tfs_.transform.rotation.x = 0; // it.transform().rotation().qx();
    tfs_.transform.rotation.y = 0; // it.transform().rotation().qy();
    tfs_.transform.rotation.z = 0; // it.transform().rotation().qz();
    tfs_.transform.rotation.w = 1; // it.transform().rotation().qw();
...
int main(int argc, char *argv[]) {
  ros::init(argc, argv, "tfstatic2"); //节点名为study，随便取
  ros::NodeHandle n;
  struct para_t para;
