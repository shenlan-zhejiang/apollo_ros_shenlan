...
class CarlaRosBridge(CompatibleNode):
    ...
    def initialize_bridge(self, carla_world, params):
        ...
        # set spectator pose if any
        if len(params['spectator_pose']) == 6:
            pose = params['spectator_pose']
            trans = carla.Transform(carla.Location(pose[0], -pose[1], pose[2]),
                                    carla.Rotation(roll=pose[3],
                                                   pitch=-pose[4],
                                                   yaw=-pose[5]))
            self.carla_spectator = self.carla_world.get_spectator()
            self.loginfo("Set spectator to pose : {}".format(trans))
            self.carla_spectator.set_transform(trans)
    ...
    def _synchronous_mode_update(self):
        ...
        """
        if self.carla_settings.fixed_delta_seconds:
            loop_dur = self.carla_settings.fixed_delta_seconds
        else: 
            loop_dur = 0    
        while not self.shutdown.is_set() and roscomp.ok():
            ...
            need_sleep = max(0, loop_dur - (time.time() - start_time))
...
def main(args=None):
    ...
    parameters["spectator_pose"] = carla_bridge.get_param('spectator_pose', [])
    ...
    try:
        ...
        if "town" in parameters and not parameters['passive']:
            ...
            # carla_world.load_map_layer(carla.MapLayer.Ground)
            carla_world.tick()
