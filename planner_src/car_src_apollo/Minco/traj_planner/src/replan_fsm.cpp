...
void ReplanFSM::init(ros::NodeHandle &nh)
{
    ...
    // int car_id = 0;
    // nh.param("planning/car_id", car_id, car_id);
    // std::string minco_config_path = "traj_planner/config/minco_config.pb.txt";
    // nh.param("planning/minco_config_path", minco_config_path, minco_config_path);
    // cout << "config_path: " << minco_config_path << endl;
...
void ReplanFSM::OdomCallback(const nav_msgs::Odometry& msg)
{
    ...
    // @ROCY: APOLLO
    Eigen::Vector3d pos2center(0, -car_d_cr_, 0);
...
{
    // cout << "car_id from the swarm_bridge: " << traj_msg.car_id << endl;
...
void ReplanFSM::execFSMCallback(const ros::TimerEvent &e)
{
    ...
    switch (exec_state_)
    {
        ...
        case SEQUENTIAL_START:
        {
            ...
            //std::cout << "case6 start_time" << start_time << std::endl;

            start_world_time_ = start_time;
            
            ros::Time t1 = ros::Time::now();
            //std::cout << "case6 t1" << t1 << std::endl;
            ...
            //std::cout << "case6 time_spent_in_planning" << time_spent_in_planning << std::endl;
        ...
        case EXEC_TRAJ:
        {
            ...
            // std::cout << "t_now.toSec() - start_world_time_ > 2.5: "<< std::endl << t_now.toSec() - start_world_time_<< " " << t_now.toSec() << " " << start_world_time_ << std::endl;
            ...
            {
                // std::cout << "t_now.toSec() - start_world_time_ > TIME_BUDGET5: " << t_now.toSec() - start_world_time_ <<std::endl;
                changeFSMExecState(REPLAN_TRAJ, "FSM");
                collision_with_obs_ = false;
                collision_with_othercars_ = false;
                break;
