...
void MappingProcess::init(const ros::NodeHandle& nh)
{
    ...
    // pointcloud_odom_sync_->registerCallback(boost::bind(&MappingProcess::Pointcloud_cb, this, _1, _2));
    sub = nh_.subscribe<sensor_msgs::PointCloud2>("/lidar", 1000, boost::bind(&MappingProcess::Pointcloud_cb, this, _1));
    ...
    pcd_odom.open("/home/shenlan/apollo/modules/map/data/beijing/parsed_data_pillar/00000/pcd/corrected_poses.txt");
    odom_msg = std::make_shared<nav_msgs::Odometry>();
    std::cout << "MappingProcess Initialized!\n" << endl;

}

void parse(ifstream &file, std::shared_ptr<nav_msgs::Odometry> &odom_msg)
{
    int id;
    double time_stamp;
    double x, y, z, qw, qx, qy, qz;
    file >> id >> time_stamp >> x >> y >> z >> qx >> qy >> qz >> qw;
    ...
    odom_msg->child_frame_id = "agent_0";
    odom_msg->pose.pose.position.x = x + 67814;
    odom_msg->pose.pose.position.y = y - 4457770;
    odom_msg->pose.pose.position.z = z - 33;
    odom_msg->pose.pose.orientation.x = qx;
    odom_msg->pose.pose.orientation.y = qy;
    odom_msg->pose.pose.orientation.z = qz;
    odom_msg->pose.pose.orientation.w = qw;
    std::cout << id << "parse" << odom_msg->pose.pose << std::endl;
}
...
void MappingProcess::Pointcloud_cb(const sensor_msgs::PointCloud2::ConstPtr &pcl_msg)
{
    ...
    parse(pcd_odom, odom_msg);
    latest_odom_time_ = pcl_msg->header.stamp;
