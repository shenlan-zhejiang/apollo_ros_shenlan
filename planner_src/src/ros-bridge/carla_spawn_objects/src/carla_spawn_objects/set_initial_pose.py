...
class SetInitialPose(CompatibleNode):
    ...
    def intial_pose_callback(self, initial_pose):
        mat44 = numpy.eye(4)
        mat44[:3, :3] = quat2mat([initial_pose.pose.pose.orientation.w, initial_pose.pose.pose.orientation.x,
                                initial_pose.pose.pose.orientation.y, initial_pose.pose.pose.orientation.z])
        b = numpy.zeros((3, 1))
        b[0, 0] = 1.38641
        a = mat44[:3, :3].dot(b)
        initial_pose.pose.pose.position.x += a[0, 0]
        initial_pose.pose.pose.position.y += a[1, 0]
        initial_pose.pose.pose.position.z += a[2, 0]
