...
namespace path_searching
{
  ...
  void KinoAstar::init(ros::NodeHandle& nh)
  {
    ...
    // checkl = cfg_.map_cfg().checkl();
      ...
      // max_steer_ = std::atan(car_wheelbase_ * max_cur_);
  ...
  // void KinoAstar::resetEndState(Eigen::Vector4d& start_state, Eigen::Vector4d& end_state){
  //    /*
  ...
  //   double Limit_len = 15.0;
  //   namespace ob = ompl::base;
  //   namespace og = ompl::geometric;
  //   ob::ScopedState<> from(shotptr), to(shotptr), s(shotptr);
  //   from[0] = start_state[0]; from[1] = start_state[1]; from[2] = start_state[2];
  //   to[0] = end_state[0]; to[1] = end_state[1]; to[2] = end_state[2];
  //   std::vector<double> reals;
  //   double totalLen = shotptr->distance(from(), to());
  //   bool isocc = false;
  //   Eigen::Vector3d state; 
  //   if(totalLen <= Limit_len) return;
  //   else{
  //     shotptr->interpolate(from(), to(), Limit_len/totalLen, s());
  //     reals = s.reals();
  //     state<<reals[0],reals[1],reals[2];
  //     frontend_map_itf_->CheckIfCollisionUsingPosAndYaw(vp_,state,&isocc);
  //     if(!isocc){
  //       end_state << state[0],state[1],state[2],0.0;
  //       return;
  //     } 
  //     //find the local target which is nearest to the max_len
  //     double left = Limit_len - checkl; double right = Limit_len + checkl;
  //     while(true){
  //       // std::cout<<"left: "<<left<<" right: "<<right<<" totallen: "<<totalLen<<std::endl;
  //       //check left
  //       shotptr->interpolate(from(), to(), left/totalLen, s());
  //       reals = s.reals();
  //       state<<reals[0],reals[1],reals[2];
  //       frontend_map_itf_->CheckIfCollisionUsingPosAndYaw(vp_,state,&isocc);
  //       if(!isocc){
  ...
  //       }
  //       //check right
  //       shotptr->interpolate(from(), to(), right/totalLen, s());
  //       reals = s.reals();
  //       state<<reals[0],reals[1],reals[2];
  //       frontend_map_itf_->CheckIfCollisionUsingPosAndYaw(vp_,state,&isocc);
  //       if(!isocc){
  ...
  //       left-=checkl;
  //       right+=checkl;
  //       left = std::max(checkl,left);
  //       right = std::min(totalLen,right);
  //       if(left<=checkl&&right>=totalLen){
  //         ROS_WARN("reset end point failed");
  //         return;
  //       }
  //     }
  //   }
  ...
  void KinoAstar::getTruncatedposLists()
  {
      ...
      // double tmp_len = 0.0;
      // cout << "number of kino nodes: " << path_nodes_.size() << endl;
      // // positionList_.push_back(start_pos);

      // Eigen::Vector2d final_start_pos, final_end_pos; // the start and end position of final node
      ...
      //     final_start_pos = path_nodes_[path_nodes_.size()-2]->state.head(2);
      //     final_end_pos = path_nodes_.back()->state.head(2);
      // }
      // else if(path_nodes_.size() == 2)
      // {
      //     final_start_pos = start_pos;
      //     final_end_pos = path_nodes_.back()->state.head(2);
      //     positionList_.push_back(final_start_pos);
      //     positionList_.push_back(final_end_pos);
      // }
      // else
      // {
      //     final_start_pos = final_end_pos = start_pos;
      //     positionList_.push_back(start_pos);
      // }
      ...
      //     std::vector<double> reals;
      //     Eigen::Vector2d last_pos = final_end_pos;
      //     for(double l = checkl; l < shotLength; l += checkl)
      //     {
      //         shotptr->interpolate(from(), to(), l / shotLength, s());
      //         reals = s.reals();
      //         Eigen::Vector2d cur_pos(reals[0], reals[1]);
      //         // Eigen::Vector2d last_direction = cur_pos - last_pos;
      //         if(l <= checkl + 1e-3)
      //         {
      //             positionList_.push_back(cur_pos);
      //         }
      //         else
      //         {
      //             if(l + checkl < shotLength)
      //             {
      //                 shotptr->interpolate(from(), to(), (l + checkl) / shotLength, s());
      //                 reals = s.reals();
      //                 Eigen::Vector2d next_pos(reals[0], reals[1]);
      //                 Eigen::Vector2d next_direction = next_pos - cur_pos;
      //                 Eigen::Vector2d cur_direction = cur_pos - last_pos;
      //                 if(cur_direction.dot(next_direction) > 0)
      //                 {
      //                     if((next_pos - cur_pos).norm() < 0.8 * checkl)
      //                     {
      //                         continue;
      //                     }
      //                     positionList_.push_back(cur_pos);
      //                 }
      //                 else if(cur_direction.dot(next_direction) <= 0)
      //                 {
      //                     if((cur_pos - last_pos).norm() < 0.8 * checkl)
      //                     {
      ...
      //             else
      //             {
      //                 Eigen::Vector2d end_pos = end_state_.head(2);
      //                 if((end_pos - cur_pos).norm() < 0.8 * checkl /*&& (cur_pos - last_pos).dot(end_pos - cur_pos) > 0*/)
      //                 {
      //                     positionList_.push_back(end_pos);
      //                 }
      //                 else
      //                 {
      //                     positionList_.push_back(cur_pos);
  ...
  bool KinoAstar::searchTime(plan_utils::KinoTrajData &flat_trajs, double &start_world_time)
  {
      ...
      for(double dis = distance_resolution_; dis < distance; dis += distance_resolution_)
      {
          int locate_idx = 0;
          for(int i = 1; i <= end_index; i++)
          {
      ...
      // {
      //     cout << "positionList_.x: " << positionList_[i](0) << "positionList_.y: " << positionList_[i](1) << endl;
      // }

      // for(int i = 0; i < discrete_positionList.size(); i++)
      // {
      //     cout << "dis_pos.x: " << discrete_positionList[i](0) << "dis_pos.y: " << discrete_positionList[i](1) << endl;
      // }
      /*------------------------------------------------------------------*/
      
      /*build s-t graph step2: enumurate every point of other car's trajectory and check collision*/
      double collision_distance = car_width_ / 2.0;
      ...
      for(int sur_id = 0; sur_id < cars_num_; sur_id++)
      {
          ...
          for(double world_t = start_world_time; world_t < traj_end_time; world_t += time_resolution_)
          {
              ...
              Eigen::Vector2d surround_p;
              surround_p = surround_segment->getPos(pt_time);
              for(int s_int = 0; s_int < discrete_positionList.size(); s_int++)
              {
      ...
      int max_int_s = discrete_positionList.size();
      int max_int_t = max_int_time;
      int max_voxels = max_int_s * max_int_t;
      ...
      {
          int time_int = s_t_graph[i](0);
          int distance_int = s_t_graph[i](1);
          int time_distance_int = time_int + distance_int * max_int_t;
      ...
      /*----------------------------------------------------------*/
      bool use_middle_node = false;
      int decrease_at = 0;

      int start_direction;
      double plan_start_vel; // attention: plan_start_vel should not be zero
      ...
      if(abs(start_state_[3]) <= non_siguav)
      {
          plan_start_vel = (start_direction > 0) ? non_siguav : -non_siguav;
      }
      else
      {
          plan_start_vel = (start_direction > 0) ? abs(start_state_(3)) : -abs(start_state_(3));
      }


      // if(start_state_(3) > non_siguav)
      // {
      //     start_direction = 1;
      //     plan_start_vel = start_state_(3);
      // }
      // else if(start_state_(3) < -non_siguav)
      // {
      //     start_direction = -1;
      //     plan_start_vel = start_state_(3);
      // }
      ...
      //     }
      // }
      cout << "replan_start_singul: " << start_direction << endl;
      cout << "replan_start_vel: " << plan_start_vel << endl;
      // cout << "number of direction change nodes: " << direction_change_idx_.size() << endl;
      cout << "number of position list: " << positionList_.size() << endl;
      for(int i = 0; i < direction_change_idx_.size(); i++)
      {
          cout << "direction change index: " << direction_change_idx_[i] << endl;
      }
      

      // reset for time search
      ...
      start_node->start_pos = positionList_[0];
      start_node->end_pos = positionList_[0];
      start_node->start_vel = start_state_[3];
      start_node->end_vel = plan_start_vel;
      start_node->yaw = 0.0;
      start_node->duration = 0.0;
      start_node->singul = start_direction;
      start_node->node_idx = 0;
      start_node->acc = 0.0;
      start_node->length = 0.0;
      start_node->g_score = 0.0;
      start_node->f_score = gettimeHeu(start_node->node_idx, start_node->end_vel);
      start_node->start_time = start_world_time;
      ...
      double length_to_first_singul_point = calculateCurNode2nearestSingul(1);
      if(plan_start_vel * plan_start_vel / (2 * max_acc_) > length_to_first_singul_point)
          max_input = plan_start_vel * plan_start_vel / (2 * length_to_first_singul_point) + 0.1;// max_input = 1.5 * max_acc_; 

      /*-------------------for debugging-----------------------*/
      int debug_max_index = 1;
      std::vector<Eigen::Vector4d> debug_failed_singul_nodes;
      debug_failed_singul_nodes.clear();

      std::vector<Eigen::Vector4d> debug_openset_nodes;
      debug_openset_nodes.clear();
      /*-------------------------------------------------------*/
      while( !openSet_.empty() )
      {
          ...
          // if(cur_node->node_idx == positionList_.size() - 1)
          // {
          //     // stop searching, reach the final point!
          //     MiddleNodePtr node = cur_node;
          ...
          //     cout << "search success! " << "use_node_num: " << use_time_node_num_ << endl;
          //     for(int i = 0; i < middle_nodes_.size(); i++)
          //     {
          //         cout << " node_idx: " << middle_nodes_[i]->node_idx;
          //         if(middle_nodes_[i]->node_idx == path_nodes_.size())
          //         {
          //             ROS_WARN("one shot start here! But I successfully search out!");
          //         }
          //         cout << " start_vel: " << middle_nodes_[i]->start_vel;
          //         cout << " end_vel: " << middle_nodes_[i]->end_vel;
          //         cout << " acc: " << middle_nodes_[i]->acc;
          //         cout << " duration: " << middle_nodes_[i]->duration << endl;
          //     }
          ...
          // double input;
          std::vector<double> ctrl_inputs;
          double res = 0.5;
          for(double input = -max_input; input < max_input + 1e-3; input += res * max_input)
          {
              ctrl_inputs.push_back(input);
          }
          int pro_node_idx = cur_node->node_idx + 1;
          if(debug_max_index < pro_node_idx)
          {
              debug_max_index = pro_node_idx;
          }
          // cout << "expanded node index: " << pro_node_idx << " " << positionList_.size() << endl;
 
          Eigen::Vector2d pro_start_pos = positionList_.at(pro_node_idx - 1);
          Eigen::Vector2d pro_end_pos = positionList_.at(pro_node_idx);

          double pro_node_length = (pro_end_pos - pro_start_pos).norm();
          double pro_start_time = cur_node->start_time + cur_node->duration;
          int pro_singul;
          bool arrive_at_the_singul_point = false;
          int nearest_next_singul_idx;

          for(int i = 0; i < direction_change_idx_.size() - 1; i++)
          {
              if(pro_node_idx > direction_change_idx_.at(i) && pro_node_idx <= direction_change_idx_.at(i+1))
              {
                  pro_singul = pow(-1, i) * start_direction;
                  nearest_next_singul_idx = direction_change_idx_.at(i+1);
              }
              if(pro_node_idx == direction_change_idx_.at(i+1))
              {
                  arrive_at_the_singul_point = true;
              }
          }

          double distance_to_singul_point = 0.0;
          if(!arrive_at_the_singul_point)
          {
              for(int idx = cur_node->node_idx; idx < nearest_next_singul_idx; idx++)
              {
                  distance_to_singul_point += (positionList_[idx+1] - positionList_[idx]).norm();
              }                  
          }

          // double pro_start_vel = cur_node->end_vel;
          double pro_start_vel;
          if(cur_node->singul * pro_singul < 0)
              pro_start_vel = -cur_node->end_vel;
          else 
              pro_start_vel = cur_node->end_vel;

          for(double input : ctrl_inputs)
          {
              double pro_acc = input;  
              double pro_end_vel, pro_input, duration;
              if(!arrive_at_the_singul_point)
              {
                  double node_vel = pro_singul * pro_start_vel;
                  double node_acc = pro_singul * input; 
                  // no solution, skip this input
                  if(node_vel * node_vel + 2 * node_acc * pro_node_length < 0)
                      continue;
                  
                  if(node_acc > 0)
                      duration = (-node_vel + sqrt(node_vel * node_vel + 2 * node_acc * pro_node_length)) / node_acc;
                  else if(node_acc == 0)
                      duration = pro_node_length / node_vel;
                  else
                      duration = (-node_vel + sqrt(node_vel * node_vel + 2 * node_acc * pro_node_length)) / node_acc;

                  // exceed the max speed or the direction of speed changed, skip this input
                  if(node_vel + node_acc * duration <= non_siguav || node_vel + node_acc * duration > max_vel_)
                      continue;
                  

                  pro_end_vel = pro_start_vel + input * duration;
                  pro_input = input;

                  double distance_from_pro_node_end_to_nearest_singul = distance_to_singul_point - pro_node_length;
                  if(pro_end_vel * pro_end_vel / (2 /** 0.8 */* max_input) > distance_from_pro_node_end_to_nearest_singul)
                      continue;

                  double pro_end_time = pro_start_time + duration;
                  // bool collision_with_other_cars = false;
                  // for(int sur_id = 0; sur_id < cars_num_; sur_id++)
                  // {
                  //     if(sur_id == car_id_ || !ifdynamic_)
                  //         continue;
                  ...
                  //     double pt_time;
                  //     plan_utils::Trajectory* surround_segment;
                  //     int sur_segmentId = swarm_traj_container_[sur_id].locateSingulId(pro_end_time);
                  ...
                  //     if(pro_end_time < sur_segment_start_time)
                  //     {
                  //         int sur_last_segmentId = swarm_last_traj_container_[sur_id].locateSingulId(pro_end_time);
                  ...
                  //         if(pro_end_time < sur_last_segment_start_time)
                  //         {
                  //             pt_time = 0.0;
                  //             surround_segment = &swarm_last_traj_container_[sur_id].singul_traj[sur_last_segmentId].traj;
                  //         }
                  //         else if(pro_end_time > sur_last_segment_end_time)
                  //         {
                  ...
                  //         else
                  //         {
                  //             pt_time = pro_end_time - sur_last_segment_start_time;
                  //             surround_segment = &swarm_last_traj_container_[sur_id].singul_traj[sur_last_segmentId].traj;
                  //         }
                  //     }
                  //     else if(pro_end_time > sur_segment_end_time)
                  //     {
                  //         pt_time = swarm_traj_container_[sur_id].singul_traj[sur_segmentId].duration;
                  //         surround_segment = &swarm_traj_container_[sur_id].singul_traj[sur_segmentId].traj;
                  //     }
                  //     else
                  //     {
                  //         pt_time = pro_end_time - sur_segment_start_time;
                  //         surround_segment = &swarm_traj_container_[sur_id].singul_traj[sur_segmentId].traj;
                  //     }
                  ...
                  //     if((surround_p - pro_end_pos).norm() < car_width_ / 2.0)
                  //     {
                  //         collision_with_other_cars = true;
                  //         use_middle_node = true;
                  //         decrease_at = pro_node_idx;
                  //         // ROS_ERROR("collision with other cars! break!");
                  //         break;
                  //     }

                  // }

                  // if(collision_with_other_cars)
                  //     continue;

                  /*todo: if collision with other cars， continue*/
                  // if (collision with other cars)
                  // {
                  //     continue;
                  // } 
                  /*---------------------------------------------*/
              ...
              // This is the special case, there is only one primitive between two singul points
              // for this kind of case, we think the car moves uniformly in a speed of 1m/s
              // and we only allocate one time node here, which means we eliminate every node whose 
              // acceleration is not 0.0 m/s^2
              else if(arrive_at_the_singul_point && abs(pro_start_vel) <= non_siguav)
              {
                  double node_vel = pro_singul * pro_start_vel;
                  double node_acc = pro_singul * input;
                  if(abs(node_acc) > 0.05)
                      continue;

                  duration = pro_node_length / 1.0;
                  ...
                  // } 
                  /*---------------------------------------------*/    
                  pro_end_vel = (pro_singul > 0) ? non_siguav : -non_siguav;
                  pro_input = 0.0;                 
              }
              // for this case, the car is moving towards the singul point, hence the 
              // car is going to stop at the singul point. And because of this, the duration
              // is not decided by the input we sample before. As we have known the velocity first, 
              // so we only reserve one time node.
              else if(arrive_at_the_singul_point && abs(pro_start_vel) > non_siguav)
              {
                  double node_vel = pro_singul * pro_start_vel;
                  double node_acc = pro_singul * input;
                  // near the singul point, the car must decrease its speed, so we only reserve one node here
                  if(abs(node_acc) >= 0.05)
                      continue;
                  
                  node_acc = node_vel * node_vel / (2 * pro_node_length);
                  Eigen::Vector4d debug_singul_node;
                  debug_singul_node << node_acc, node_vel, cur_node->f_score, cur_node->g_score + node_vel / node_acc + 0.05 * pow(node_acc - cur_node->acc, 2) + gettimeHeu(pro_node_idx, non_siguav);
                  debug_failed_singul_nodes.push_back(debug_singul_node);

                  // max / 6.5 is to make it not to stop tooooo slow         a damn trick here which wastes me nearly a weak
                                                                          // just for a segment only have two nodes
                  if(node_acc >  2 * max_input || node_acc < max_input / 6.5 && abs(cur_node->start_vel) > non_siguav)
                      continue;
                  
                  duration = node_vel / node_acc;
                  ...
                  pro_end_vel = (pro_singul > 0) ? non_siguav : -non_siguav;
                  pro_input = (pro_singul > 0) ? -node_acc : node_acc;
              }

              MiddleNodePtr pro_node;
              pro_node = middle_node_pool_[use_time_node_num_];
              pro_node->start_pos = pro_start_pos;
              pro_node->end_pos = pro_end_pos;
              pro_node->start_vel = pro_start_vel;
              pro_node->end_vel = pro_end_vel;
              pro_node->yaw = atan2(pro_singul * (pro_end_pos - pro_start_pos)[1], pro_singul * (pro_end_pos - pro_start_pos)[0]);
              // pro_node->yaw = normalize_angle(pro_node->yaw); // The range of yaw should between -π to π
              pro_node->duration = duration;
              pro_node->singul = pro_singul;
              pro_node->node_idx = pro_node_idx;
              pro_node->acc = pro_input;
              pro_node->length = pro_node_length;
              pro_node->g_score = cur_node->g_score + duration + 0.05 * pow(pro_node->acc - cur_node->acc, 2);
              pro_node->f_score = pro_node->g_score + gettimeHeu(pro_node->node_idx, pro_node->end_vel);
              pro_node->start_time = pro_start_time;
              pro_node->parent = cur_node;

              Eigen::Vector4d expanded_debug_openset_node;
              expanded_debug_openset_node << (double)pro_node_idx, pro_start_vel, pro_input, distance_to_singul_point;
              debug_openset_nodes.push_back(expanded_debug_openset_node);

              use_time_node_num_++;
              if(use_time_node_num_ == allocate_num_)
              {
                  ROS_ERROR("too many nodes! Search fails!");
                  cout << "max_searched index: " << debug_max_index << endl;
                  cout << "The number of node numbers: " << positionList_.size() << endl;
                  for(int i = 0; i < positionList_.size(); i++)
                  {
                      if(i == path_nodes_.size())
                      {
                          ROS_WARN("one shot start here！");
                      }
                      for(int j = 1; j < direction_change_idx_.size(); j++)
                      {
                          if(i == direction_change_idx_[j])
                          {
                              ROS_WARN("direction change here!");
                          }
                      }
                      cout << "x: " << positionList_[i](0) << " y: " << positionList_[i](1) << endl;
                  }
                  ...
                  cout << "debug last singul node" << endl;
                  for(int i = 0; i < debug_failed_singul_nodes.size(); i++)
                  {
                      cout << "failed acc: " << debug_failed_singul_nodes[i](0) << " failed start vel: " << debug_failed_singul_nodes[i](1);
                      cout << " cur_fscore: " << debug_failed_singul_nodes[i](2) << "pro_fscore: " << debug_failed_singul_nodes[i](3) << endl;
                  }

                  // reverse(middle_nodes_.begin(), middle_nodes_.end());
                  // cout << "THe number of searched node numbers: " << middle_nodes_.size() << endl;                     
                  // for(int i = 0; i < middle_nodes_.size(); i++)
                  // {
                  //     cout << " node_idx: " << middle_nodes_[i]->node_idx;
                  //     cout << " start_vel: " << middle_nodes_[i]->start_vel;
                  //     cout << " end_vel: " << middle_nodes_[i]->end_vel;
                  //     cout << " acc: " << middle_nodes_[i]->acc;
                  //     cout << " duration: " << middle_nodes_[i]->duration;
                  //     cout << " distance to final end: " << (positionList_.back() - middle_nodes_[i]->start_pos).norm() << endl;
                  // }
                  return false;
              }

              if(pro_node->node_idx == positionList_.size() - 1)
              {
                  // stop searching, reach the final point!
                  MiddleNodePtr node = pro_node;
                  ...
                  /*for serving the init values for later optimization process*/
                  flat_trajs.clear();
                  for(int i = 0; i < direction_change_idx_.size() - 1; i++)
                  {
                      int start_idx = direction_change_idx_[i] + 1;
                      int end_idx = direction_change_idx_[i+1];
                      int segment_singul = middle_nodes_[start_idx]->singul;
                      Eigen::Vector2d segment_start_pos = middle_nodes_[start_idx]->start_pos;
                      Eigen::Vector2d segment_end_pos = middle_nodes_[end_idx]->end_pos;
                      double segment_start_vel = middle_nodes_[start_idx]->start_vel;
                      double segment_end_vel = middle_nodes_[end_idx]->end_vel;
                      double segment_start_time = middle_nodes_[start_idx]->start_time;

                      /*This part aims to make the singul point between two segments to be smooth yaw*/ 
                      double segment_start_yaw = middle_nodes_[start_idx]->yaw;
                      double segment_end_yaw = middle_nodes_[end_idx]->yaw;
                      double last_segment_end_yaw = segment_start_yaw;
                      double next_segment_start_yaw = segment_end_yaw;
                      if(i != 0)
                      {
                          last_segment_end_yaw = middle_nodes_[start_idx-1]->yaw;
                      }
                      if(i != direction_change_idx_.size() - 2)
                      {
                          next_segment_start_yaw = middle_nodes_[end_idx+1]->yaw;
                      }
                      cout << "---------------------------------" << endl;
                      cout << "segment start yaw: " << segment_start_yaw << endl;
                      cout << "last segment end yaw： " << last_segment_end_yaw << endl;
                      cout << "next segment start yaw: " << next_segment_start_yaw << endl;
                      cout << "segment end yaw: " << segment_end_yaw << endl;
                      if(   (segment_start_yaw > M_PI / 2.0 && last_segment_end_yaw < -M_PI / 2.0)
                         || (last_segment_end_yaw > M_PI / 2.0 && segment_start_yaw < -M_PI / 2.0) )
                      {
                          double average_yaw = (segment_start_yaw + last_segment_end_yaw) / 2.0;
                          if(average_yaw < 0)
                          {
                              segment_start_yaw = normalize_angle(average_yaw + M_PI);
                          }
                          else
                          {
                              segment_start_yaw = normalize_angle(average_yaw - M_PI);
                          }
                          // segment_start_yaw = normalize_angle((segment_start_yaw + last_segment_end_yaw) / 2.0 + pi);
                      }
                      else
                      {
                          segment_start_yaw = (segment_start_yaw + last_segment_end_yaw) / 2.0;
                      }
                      
                      if(   (segment_end_yaw > M_PI / 2.0 && next_segment_start_yaw < -M_PI / 2.0) 
                         || (segment_end_yaw < -M_PI / 2.0 && next_segment_start_yaw > M_PI / 2.0) )
                      {
                          double average_yaw = (segment_end_yaw + next_segment_start_yaw) / 2.0; 
                          if(average_yaw < 0)
                          {
                              segment_end_yaw = normalize_angle(average_yaw + M_PI);
                          }
                          else
                          {
                              segment_end_yaw = normalize_angle(average_yaw - M_PI);
                          }
                          // segment_end_yaw = normalize_angle((segment_end_yaw + next_segment_start_yaw) / 2.0 + pi);
                      }
                      else
                      {
                          segment_end_yaw = (segment_end_yaw + next_segment_start_yaw) / 2.0;
                      }
                      ...
                      {
                          segment_duration += middle_nodes_[j]->duration;
                      }
                      ...
                      for(samplet = sampletime; samplet < segment_duration; samplet += sampletime)
                      {
                          ...
                          for(int j = start_idx; j <= end_idx; j++)
                          {
                              if(samplet > middle_nodes_[j]->start_time - segment_start_time
                                 && samplet <= middle_nodes_[j]->start_time + middle_nodes_[j]->duration - segment_start_time)
                              {
                                  locate_node_idx = j;
                                  break;
                              }
                          }
                          double t_bar = samplet + segment_start_time - middle_nodes_[locate_node_idx]->start_time;
                          int node_singul = middle_nodes_[locate_node_idx]->singul;
                          double node_start_vel = node_singul * middle_nodes_[locate_node_idx]->start_vel;
                          double node_end_vel = node_singul * middle_nodes_[locate_node_idx]->end_vel;
                          double node_acc = node_singul * middle_nodes_[locate_node_idx]->acc;
                          double node_length = middle_nodes_[locate_node_idx]->length;

                          // for the specific case, the segment only includes one node
                          if(node_start_vel <= non_siguav && node_end_vel <= non_siguav)
                          {
                              node_start_vel = 1.0;
                              node_end_vel = 1.0;
                          }

                          double node_yaw = middle_nodes_[locate_node_idx]->yaw;
                          double sample_length = node_start_vel * t_bar + 0.5 * node_acc * pow(t_bar, 2);
                          Eigen::Vector2d node_start_pos = middle_nodes_[locate_node_idx]->start_pos;
                          Eigen::Vector2d node_end_pos = middle_nodes_[locate_node_idx]->end_pos;
                          Eigen::Vector2d node_direction = node_end_pos - node_start_pos;
                          Eigen::Vector2d sample_pos = node_start_pos + sample_length / node_length * node_direction;
                          Eigen::Vector3d sample_point; 
                          sample_point << sample_pos, sampletime;
                          traj_pts.push_back(sample_point);
                          thetas.push_back(normalize_angle(node_yaw));
                      }
                      traj_pts.push_back(Eigen::Vector3d(segment_end_pos(0), segment_end_pos(1), segment_duration - samplet /*+ sampletime*/));
                      thetas.push_back(segment_end_yaw);

                      Eigen::Vector4d segment_start_state, segment_end_state;
                      if(i == 0)
                          segment_start_state = start_state_;
                      else
                          segment_start_state << segment_start_pos, segment_start_yaw, segment_start_vel;
                      // attention here, the kino path has been truncated, so the end state should not be end_state!
                      segment_end_state << segment_end_pos, segment_end_yaw, segment_end_vel;
                      ...
                      getFlatState(segment_start_state, Initctrlinput, startS, segment_singul);
                      getFlatState(segment_end_state, Finctrlinput, endS, segment_singul);
                      flat_traj.traj_pts = traj_pts;
                      flat_traj.thetas = thetas;
                      flat_traj.start_state = startS;
                      flat_traj.final_state = endS;
                      flat_traj.singul = segment_singul;
                      flat_trajs.push_back(flat_traj);
                  }
                  if(use_middle_node)
                  {
                      ROS_ERROR("USE middle node!!!!");
                      cout << "decrease at: " << decrease_at << endl;
                  }
      ...
      ROS_ERROR("Openset Empty! Search fails!");
      return false;
  }
  ...
  Eigen::Vector3d KinoAstar::CalculateInitPos(double& t, int& seg_idx)
  {
      int segment_start_idx = direction_change_idx_[seg_idx] + 1;
      int segment_end_idx = direction_change_idx_[seg_idx + 1];
      int segment_singul = middle_nodes_[segment_start_idx]->singul;
      int locate_node_idx;
      double node_duration_sum = 0.0;
      for(int idx = segment_start_idx; idx <= segment_end_idx; idx++)
      {
          double node_duration = middle_nodes_[idx]->duration;
          if(t >= node_duration_sum && t < node_duration_sum + node_duration + 1e-3)
          {
              locate_node_idx = idx;
              break;
          }
          node_duration_sum += node_duration;
      }
      double t_bar = t - node_duration_sum;
      double node_start_vel = middle_nodes_[locate_node_idx]->start_vel * segment_singul;
      double node_end_vel = middle_nodes_[locate_node_idx]->end_vel * segment_singul;
      double node_acc = middle_nodes_[locate_node_idx]->acc * segment_singul;
      double node_yaw = middle_nodes_[locate_node_idx]->yaw;
      Eigen::Vector2d node_start_pos = middle_nodes_[locate_node_idx]->start_pos;
      Eigen::Vector2d node_end_pos = middle_nodes_[locate_node_idx]->end_pos;
      Eigen::Vector2d node_direction = node_end_pos - node_start_pos;
      ...
      // for the specific case, one segment only includes one node, the speed is 1m/s
      if(node_start_vel <= non_siguav && node_end_vel <= non_siguav)
      {
          sample_pos = node_start_pos + 1.0 * t_bar / node_length * node_direction;
      }
      /*--------------------------------------------------------*/
      Eigen::Vector3d InitPos;
      InitPos << sample_pos, normalize_angle(node_yaw);
      return InitPos;
  }
