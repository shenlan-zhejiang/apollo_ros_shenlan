...
bool send(const std::string &remote_ip, uint16_t remote_port, uint32_t count) {
  ...
  for (uint32_t i = 0; i < count; i++) {
    ...
    pb_msg->mutable_header()->set_sequence_num(i);
    pb_msg->mutable_header()->set_timestamp_sec(timestamp_);
    //pb_msg->pose().position().set_x(0.123456 * i);
    apollo::localization::Pose* pose = new apollo::localization::Pose;
    apollo::common::PointENU* position = new apollo::common::PointENU;
    position->set_x(0.123456 * (double)i);
    pose->set_allocated_position(position);
    pb_msg->set_allocated_pose(pose);
    ...
    if (res < 0) {
      AINFO << "connected server failed ";
      continue;
    }

    AINFO << "connected to server success. port [" << remote_port << "]";

    apollo::bridge::BridgeProtoSerializedBuf<apollo::localization::LocalizationEstimate> proto_buf;
    proto_buf.Serialize(pb_msg, "LocalizationEstimate");
    //delete position;
    //delete pose;
...
int main(int argc, char *argv[]) {
  ...
  //send("192.168.1.112", 8900, count);
  send("127.0.0.1", 8901, count);
  return 0;
}
