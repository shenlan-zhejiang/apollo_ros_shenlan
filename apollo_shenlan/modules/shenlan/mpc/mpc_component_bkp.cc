...
bool MpcShenlanComponent::Proc(
    const std::shared_ptr<apollo::shenlan::mpc::Nav_path> &path, 
    ...
    if (path->header().sequence_num() != path_seq) {
        path_seq = path->header().sequence_num();
        triggerCallback(path);
    }
    if (traj->header().sequence_num() != traj_seq) {
        traj_seq = traj->header().sequence_num();
        trajCallback(traj);
    }
    if (odom->header().sequence_num() != odom_seq) {
        odom_seq = odom->header().sequence_num();
        odomCallback(odom);
    }
    if (status->header().sequence_num() != status_seq) {
        status_seq = status->header().sequence_num();
