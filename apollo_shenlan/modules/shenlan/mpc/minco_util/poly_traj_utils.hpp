...
namespace minco_utils
{
    ...
    public:
        ...
        {
            return std::atan(wheel_base_*getCurv(t));  //[-PI/2, PI/2]
        }
        ...
        inline Eigen::VectorXd getStateExpPos(const double &t) const
        {
            ...
            if ( abs(otherstate[2]) < 1e-6){
              ...
              // phi
              otherstate[4] = std::atan(wheel_base_*otherstate[1]);
              // s
              otherstate[5] = 0.0;
              for (double time=0, dt=0.01; time<t; time+=dt)
              {
                otherstate[5] += getVel(time)*dt;
              }
            }
    ...
    public:
        ...
        //@yuwei
        inline void GetState(double t, State& state) const
        {
            double inner_t = t;
            ...
            Eigen::Vector2d pos_vec = pieces[pieceIdx].getPos(inner_t);
            state.x = pos_vec[0];
            state.y = pos_vec[1];

            Eigen::VectorXd otherstate = pieces[pieceIdx].getStateExpPos(inner_t);
            // theta, curv, vel, acc, phi, s;
            state.theta = otherstate(0);  // heading angle
            state.kappa = otherstate(1);
            state.v =  otherstate(2);
            state.a = otherstate(3);
            state.s = otherstate(5);// length, useless
        }
    ...
    public:
        ...
        /*
        void print() {
            std::cout << "MinJerkOpt:" << N << std::endl;
            std::cout << "headPVA:" << headPVA << std::endl;
            std::cout << "tailPVA:" << tailPVA << std::endl;
