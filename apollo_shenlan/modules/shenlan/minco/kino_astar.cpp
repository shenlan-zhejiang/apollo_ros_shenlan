...
namespace path_searching
{
  ...
  void KinoAstar::init(apollo::shenlan::ShenlanConf &shenlan_conf)
  {
      ...
      allocate_num_= shenlan_conf.kinoastar_conf().allocate_num();//100000;
      check_num_= shenlan_conf.kinoastar_conf().check_num();//5;
      max_seach_time = shenlan_conf.kinoastar_conf().max_seach_time();//1000.1;
      traj_forward_penalty = shenlan_conf.kinoastar_conf().traj_forward_penalty();//1.0;
      traj_back_penalty = shenlan_conf.kinoastar_conf().traj_back_penalty();//2.5;
      traj_gear_switch_penalty = shenlan_conf.kinoastar_conf().traj_gear_switch_penalty();//15.0;
      traj_steer_penalty = shenlan_conf.kinoastar_conf().traj_steer_penalty();//0.5;
      ...
      car_max_steering_angle_ = shenlan_conf.vehicle_conf().car_max_steering_angle();//55.0;
      ...
      max_steer_ =  shenlan_conf.vehicle_conf().car_max_steering_angle();//55.0;

      time_resolution_ = shenlan_conf.kinoastar_conf().time_resolution();//0.1;
      distance_resolution_ = shenlan_conf.kinoastar_conf().distance_resolution();//0.5;
      velocity_resolution_ = shenlan_conf.kinoastar_conf().velocity_resolution();//1.0;
      ...
      //std::cout << "KinoAstar end" << std::endl;
  }
  ...
  void posToIndex2d_self(const Eigen::Vector2d& pos, Eigen::Vector2i& id, Eigen::Vector2d& origin_, double inv_resolution_)
  {
      ...
      // std::cout << "origin_(0): " << origin_(0) << std::endl;
      // std::cout << "id(1): " << id(1) << std::endl;
      // std::cout << "pos(1): " << pos(1) << std::endl;
      // std::cout << "origin_(1): " << origin_(1) << std::endl;
  }
  ...
  int getVoxelState2d_self(const Eigen::Vector2d &pos, Eigen::Vector2d& origin_, double inv_resolution_, Eigen::Vector3i& global_map_size_, const std::shared_ptr<apollo::shenlan::OccupancyBuffer> &buf_msg)
  {
      ...
      // if (buf_msg->occupancy_buffer_2d(id(1) * global_map_size_(0) + id(0)) > 0.5)
      // {
      //     std::cout << "++++++++++++++++++" << std::endl;
      //     std::cout << "pos(0): " << pos(0) << std::endl;
      //     std::cout << "pos(1): " << pos(1) << std::endl;
  ...
  int KinoAstar::search(Eigen::Vector4d start_state, Eigen::Vector2d init_ctrl, Eigen::Vector4d end_state, bool use3d, const std::shared_ptr<apollo::shenlan::OccupancyBuffer> &buf_msg)
  {
    ...
    double t1 = apollo::cyber::Time::Now().ToSecond();
    //double t1 = Clock::NowInSeconds();
    ...
    while (!open_set_.empty())
    {
      ...
      if (is_shot_succ_)
      {
        ...
        {
          std::cout << "[Kino Astar]: Reach horizon_" << std::endl;
          return REACH_HORIZON;
        }
      }
      //ros::Time t2 = ros::Time::now();
      double t2 = apollo::cyber::Time::Now().ToSecond();
      ...
      if(runTime > max_seach_time){
        ...
        else
        {
          ...
          cout<<"KinoSearch: Reach the max seach time"<<endl;;
          return REACH_END;
        }
      }
      ...
      for (auto& input:inputs){
        ...
        {
          if(cur_node->number < 20 && cur_node->number + nearest_idx_ < (int)last_path_pos_.size())
          {
    ...
    std::cerr << "Open set empty, no path!" << std::endl;
    return NO_PATH;
  }
  ...
  bool KinoAstar::is_shot_sucess(Eigen::Vector3d state1, Eigen::Vector3d state2, const std::shared_ptr<apollo::shenlan::OccupancyBuffer> &buf_msg){
    ...
    double len,st;
    //double ct1 = apollo::cyber::Time::Now().ToNanosecond();
    st = computeShotTraj(state1,state2,path_list,len);
    //double ct2 = apollo::cyber::Time::Now().ToNanosecond();
    bool is_occ = false;
    // double t1 = ros::Time::now().toSec();
  ...
  void KinoAstar::retrievePath(PathNodePtr end_node)
  {
    ...
    for(int i = 0; i < (int)last_path_pos_temp_.size(); i++)
    {
  ...
  void KinoAstar::findNearestNode(Eigen::Vector2d& start_pos, bool first_search)
  {
    if(!first_search)
    {
      ...
      nearest_idx_ = 0;
      for(int i = 0; i < (int)last_path_pos_.size(); i++)
      {
  ...
  Eigen::Vector3d KinoAstar::evaluatePos(double t){
    ...
    for(int i = 0;i<(int)shot_timeList.size();i++){
    ...
    double tmparc = 0;
    for(int i = 0; i < (int)localTraj.size()-1;i++){
  ...
  void KinoAstar::getSingulNodes()
  {
      ...
      for(int i = 1; i < (int)positionList_.size() - 1; i++)
      {
      ...
      for(int i = 1; i < (int)direction_change_idx_.size(); i++)
      {
      ...
      for(int i = 0; i < (int)Vec_node_need_to_be_erased.size(); i++)
      {
          positionList_.erase(positionList_.begin() + Vec_node_need_to_be_erased[i]);
          // Vec_node_need_to_be_erased[i]--;
          for(int j = i + 1; j < (int)Vec_node_need_to_be_erased.size(); j++)
          {
  ...
  bool KinoAstar::searchTime(plan_utils::KinoTrajData &flat_trajs, double &start_world_time)
  {
      ...
      for(int sur_id = 0; sur_id < cars_num_; sur_id++)
      {
          ...
          for(double world_t = start_world_time; world_t <= traj_end_time + 1e-3; world_t += time_resolution_)
          {
              ...
              for(int s_int = 0; s_int < (int)discrete_positionList.size() - 1; s_int++)
              {
      ...
      for(int i = 0; i < (int)s_t_graph.size(); i++)
      {
      ...
      while(!openSet_.empty())
      {
          ...
          {
              cout<<"Too fast init speed, not possible to stop!"<<endl;
              continue;
              // return false;
          }
          ...
          if(one_shot_success)
          {
              ...
              // if(debug_middle_nodes.size() > 10 && s_t_graph.size() > 10)
              // {
              //     string directory = "/home/aolai/car_swarm_carla/st_data/car_" + std::to_string(car_id_) + ".txt";
              //     const char* directory_name = directory.c_str();
              //     std::FILE *fp = fopen(directory_name, "a+");
              //     fprintf(fp, "%0.12f ", plan_start_vel);
              //     fprintf(fp, "%0.12f ", total_s_);
              //     fprintf(fp, "1 ");
              //     fprintf(fp, "%0.12f ", shot_start_t);
              //     fprintf(fp, "%0.12f ", shot_start_s);
              //     fprintf(fp, "%0.12f \n", shot_start_vel);
              //     // fprintf(fp, "%d ", s_t_graph.size());
              //     // fprintf(fp, "%d \n", debug_middle_nodes.size());

              //     for(int i = 0; i < s_t_graph.size() - 1; i++)
              //     {
              //         fprintf(fp, "%d ", s_t_graph.at(i)(0));
              //         fprintf(fp, "%d ", s_t_graph.at(i)(1));
              //         fprintf(fp, "%d ", s_t_graph.at(i)(2));
              //     }
              //     fprintf(fp, "%d ", s_t_graph.back()(0));
              //     fprintf(fp, "%d ", s_t_graph.back()(1));
              //     fprintf(fp, "%d \n", s_t_graph.back()(2));

              //     for(int i = 0; i < debug_middle_nodes.size() - 1; i++)
              //     {
              //         fprintf(fp, "%f ", debug_middle_nodes.at(i)->start_t);
              //         fprintf(fp, "%f ", debug_middle_nodes.at(i)->start_s);
              //         fprintf(fp, "%f ", debug_middle_nodes.at(i)->end_t);
              //         fprintf(fp, "%f ", debug_middle_nodes.at(i)->end_s);
              //     }
              //     fprintf(fp, "%f ", debug_middle_nodes.back()->start_t);
              //     fprintf(fp, "%f ", debug_middle_nodes.back()->start_s);
              //     fprintf(fp, "%f ", debug_middle_nodes.back()->end_t);
              //     fprintf(fp, "%f \n", debug_middle_nodes.back()->end_s);
                  
              //     fclose(fp);
              // }

              /******todo: init values for optimizaiton*********/
          ...
          for(int i = 0; i < (int)ctrl_inputs.size(); i++)
          {
              for(int j = 0; j < (int)durations.size(); j++)
              {
                  ...
                  bool prune = false;
                  for(int k = 0; k < (int)expanded_nodes_from_one_node.size(); k++)
                  {
                  ...
                  if(!prune)
                  {
                      if(pro_node == NULL)
                      {
                          ...
                          {
                              cerr<<"Too many nodes! Middle node search fails!"<<endl;
                              return false;
                          }
      ...
      // { 
      //     cout << "t_int: " << s_t_graph.at(i)(0) << "s_int: " << s_t_graph.at(i)(1) << " car_id: " << s_t_graph.at(i)(2) << endl;
      // }
  ...
  Eigen::Vector3d KinoAstar::CalculateInitPos(double& t, int& singul)
  {
      ...
      for(int i = 0; i < (int)positionList_.size() - 1; i++)
      {
  ...
  {
      for(int i = 0; i < (int)direction_change_idx_.size() - 1; i++)
      {
  ...
  double KinoAstar::gettimeHeu(int& idx, double& vel)
  {
      ...
      for(int i = 0; i < (int)remaining_length_vector.size(); i++)
      {
  ...
  void KinoAstar::getKinoNode(plan_utils::KinoTrajData &flat_trajs)
  {
    ...
    for(int i = 0; i<(int)SampleTraj.size()-1; i++){
    ...
    for(int i=0;i<(int)shot_lengthList.size();i++){
      double initv = non_siguav,finv = non_siguav;
      ...
      if(i==(int)shot_lengthList.size() - 1) finv = endvel;
      ...
      for(samplet = sampletime; samplet<shot_timeList[i]; samplet+=sampletime){
        ...
        //find the nearest point
        for(int k = index; k<(int)localTraj.size()-1 ;k++)
        {
  ...
  double KinoAstar::evaluateDuration(double length, double startV, double endV){
   double critical_len; //the critical length of two-order optimal control, acc is the input
    if(startV>max_vel_||endV>max_vel_){
      cerr<<"kinoAstar:evaluateDuration:start or end vel is larger that the limit!"<<endl;
    }
  ...
  double KinoAstar::evaluateLength(double curt,double locallength,double localtime,double startV, double endV){
    double critical_len; //the critical length of two-order optimal control, acc is the input
    if(startV>max_vel_||endV>max_vel_){
      cerr<<"kinoAstar:evaluateLength:start or end vel is larger that the limit!"<<endl;
    }
    double startv2 = pow(startV,2);
    double endv2 = pow(endV,2);
    double maxv2 = pow(max_vel_,2);
