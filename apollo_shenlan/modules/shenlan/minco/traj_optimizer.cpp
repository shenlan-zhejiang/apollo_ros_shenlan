...
namespace plan_manage
{
      ...
      std::vector<std::vector<Eigen::MatrixXd>> &hPoly_container,std::vector<int> singuls,double now, double help_eps)
  {
    ...
    for(int i = 0; i < trajnum; i++){
      ...
      if(initInnerPts[i].cols()==0){
        cerr<<"There is only a piece?"<<endl;
        return false;
      }
      int piece_num_ = initInnerPts[i].cols() + 1;
      piece_num_container[i] = piece_num_;
      if(cfgHs_container[i].size()!=(uint)(piece_num_ - 2) * (traj_resolution_ + 1) + 2 * (destraj_resolution_ + 1)){
        std::cout<<"cfgHs size: "<<cfgHs_container[i].size()<<std::endl;
        cerr<<"cfgHs size error!"<<endl;
        return false;
      }
      ...
      // /*test whehter the first point is collision with the dynamic obstacles*/
      // Eigen::Vector2d start_pt = iniState_container[i].col(0);
      // Eigen::Vector2d end_pt = finState_container[i].col(0);
      // Eigen::Vector2d start_v = iniState_container[i].col(1);
      // Eigen::Vector2d final_v = finState_container[i].col(1);
      // int singul_i = singuls[i];
      // Eigen::Matrix2d start_R, final_R, temp_R_start, temp_R_final;
      // temp_R_start << start_v(0), -start_v(1),
      //                 start_v(1), start_v(0);
      // temp_R_final << final_v(0), -final_v(1),
      //                 final_v(1), final_v(0);
      // start_R = singul_i * temp_R_start / start_v.norm();
      // final_R = singul_i * temp_R_final / final_v.norm();

      // for(int sur_id = 0; sur_id < surround_trajs_->size(); sur_id++)
      // {
      //   int sur_singul = 1;
      //   double traj_i_start_time = surround_trajs_->at(sur_id).start_time;
      //   double start_pt_time = t_now_ - traj_i_start_time;
      // }
      
      // /*------------------------------------------------------------------------------------*/

      // /*test whether the first point and the last point is in the corridor*/
      // Eigen::MatrixXd first_corridor = cfgHs_container[i].front();
      // Eigen::MatrixXd final_corridor = cfgHs_container[i].back();
      // for(int n = 0; n < number_of_hyperplanes_of_ego_car; n++)
      // {
      //   Eigen::Vector2d le_in_world = start_R * vec_le[n] + start_pt;
      //   for(int k = 0; k < first_corridor.cols(); k++)
      //   {
      //     Eigen::Vector2d outNormal = first_corridor.col(k).head<2>();
      //     Eigen::Vector2d point_on_corridor = first_corridor.col(k).tail<2>();
      //     if(outNormal.dot(le_in_world - point_on_corridor) > 0)
      //     {
      //       ROS_ERROR("first point is not in the corridor!");
      //       return false;
      //     }
      //   }
      //   le_in_world = final_R * vec_le[n] + end_pt;
      //   for(int k = 0; k < final_corridor.cols(); k++)
      //   {
      //     Eigen::Vector2d outNormal = final_corridor.col(k).head<2>();
      //     Eigen::Vector2d point_on_corridor = final_corridor.col(k).tail<2>();
      //     if(outNormal.dot(le_in_world - point_on_corridor) > 0)
      //     {
      //       ROS_ERROR("final point is not in the corridor!");
      //       return false;
      //     }          
      //   }        
      // }
    ...
    //   if(dynamicObsCosCheck(now,iniState_container[0],0,sur_id)){
    //     ROS_ERROR("head state is in occ");
    //     return 0;
    //   }
    // }
        ...
        result == lbfgs::LBFGS_STOP||result == lbfgs::LBFGSERR_MAXIMUMITERATION)
    {
      ...
      printf("\033[32miter=%d,time(ms)=%5.3f,total_t(ms)=%5.3f,cost=%5.3f\n\033[0m", iter_num_, time_ms.count(), total_time_ms.count(), final_cost);
      cout<<"Lbfgs: The line-search routine reaches the maximum number of evaluations."<<endl;
      flag_force_return = false;
      flag_success = true;
    }
    else
  ...
  { 
   ...
   // double smoothness_cost, time_of_cost, collision_penalty, dynamic_penalty, feasibility_penalty;
    double collision_penalty,dynamic_penalty,feasibility_penalty,time_of_cost;
  ...
  void PolyTrajOptimizer::addPVAGradCost2CT(std::vector<Eigen::VectorXd>  &gdTs, Eigen::VectorXd &costs,  const int trajid, const double trajtime)
  {
    ...
    double step, alpha;
    Eigen::Matrix<double, 6, 2> gradViolaPc, gradViolaVc, gradViolaAc, gradViolaKc,gradViolaKLc,gradViolaKRc,gradViolaPhidotLc,gradViolaPhidotRc;
    ...
    double violaPosPenaD, violaVelPenaD, violaAccPenaD, violaCurPenaDL, violaCurPenaDR;
    double violaPosPena, violaVelPena, violaAccPena, violaCurPenaL, violaCurPenaR;
    ...
    for (int i = 0; i < N; ++i)
    {
      ...
      for (int j = 0; j <= K; ++j)
      {
        ...
        if (violaVel > 0.0)
        {
          ...
          gdTs[trajid](i) += omg * wei_feas_ * (violaVelPenaD * gradViolaVt * step + violaVelPena / K);
        ...
        if (violaAcc > 0.0)
        {
          ...
          gdTs[trajid](i) += omg * wei_feas_ * (violaAccPenaD * gradViolaAt * step + violaAccPena / K);
    ...
    // std::cout<<"dynamic cost: "<<costs(1)<<std::endl;
    // std::cout<<" velcost: "<<velcost<<" acccost: "<<acccost<<" curcost: "<<curcost<<"\n";
    // std::cout << "phidotcost: " << phidotcost << endl;
  }
  ...
  {
    std::cout << "*********dynamicObsGradCostP**********" << std::endl;
    ...
    for (int sur_id = 0; sur_id < cars_num_; sur_id++){
      ...
      // double pt_time_test = offsettime + t;
      // ros::Time t1 = ros::Time::now();

      // double traj_i_start_time = swarm_traj_container_[sur_id].singul_traj.at(0).start_time;
      // double offsettime = t_now_ - traj_i_start_time + trajtime;
      // // double pt_time = offsettime + t;

      // double pt_time;
      // plan_utils::Trajectory* surround_segment;
      // for(int j = 0; j < swarm_traj_container_[sur_id].singul_traj.size(); j++)
      // {
      //   double segment_j_start_time = swarm_traj_container_[sur_id].singul_traj.at(j).start_time;
      //   double segment_j_end_time = swarm_traj_container_[sur_id].singul_traj.at(j).end_time;
      //   if(t + offsettime + traj_i_start_time >= segment_j_start_time && t + offsettime + traj_i_start_time < segment_j_end_time)
      //   {
      //     pt_time = t + offsettime - (segment_j_start_time - traj_i_start_time);
      //     surround_segment = &swarm_traj_container_[sur_id].singul_traj.at(j).traj;
      //     break;
      //   }
      //   if(j == swarm_traj_container_[sur_id].singul_traj.size() - 1 && t + trajtime + traj_i_start_time >= segment_j_end_time)
      //   {
      //     pt_time = segment_j_end_time - segment_j_start_time;
      //     surround_segment = &swarm_traj_container_[sur_id].singul_traj.at(j).traj;
      //   }
      // }
      double pt_time;
      ...
      std::cout << surround_p << " " << surround_v << " " << surround_a << std::endl;
      std::cout << "*************************************" << std::endl;

      // Eigen::Matrix2d surround_R_test;
      // Eigen::Matrix2d R_u_t_hat_dot; //partial R_u over partial t_hat, which is derivative of Ru to t_hat
  ...
  void PolyTrajOptimizer::init(apollo::shenlan::ShenlanConf &shenlan_conf)
  {
      //apollo::shenlan::TrajPlannerConf trajplanner_conf;
      traj_resolution_ = shenlan_conf.trajplanner_conf().traj_resolution();//8;
      destraj_resolution_ = shenlan_conf.trajplanner_conf().destraj_resolution();//20;
      wei_obs_ = shenlan_conf.trajplanner_conf().wei_obs();//7000.0;
      wei_surround_= shenlan_conf.trajplanner_conf().wei_surround();//7000.0;
      wei_feas_ = shenlan_conf.trajplanner_conf().wei_feas();//1000.0;
      wei_sqrvar_ = shenlan_conf.trajplanner_conf().wei_sqrvar();//1000.0;
      wei_time_ = shenlan_conf.trajplanner_conf().wei_time();//500.0;
      surround_clearance_ = shenlan_conf.trajplanner_conf().surround_clearance();//1.0;
      max_vel_ = shenlan_conf.trajplanner_conf().max_vel();//6.0;
      min_vel_ = shenlan_conf.trajplanner_conf().min_vel();//-6.0;
      max_acc_ = shenlan_conf.trajplanner_conf().max_acc();//3.0;
      min_acc_ = shenlan_conf.trajplanner_conf().min_acc();//-3.0;
      max_cur_ = shenlan_conf.trajplanner_conf().max_cur();//0.3;
      half_margin = shenlan_conf.trajplanner_conf().half_margin();//0.25;
  ...
  { 
      //std::cout << "*********checkCollisionWithSurroundCars**********" << std::endl;
      ...
      for(int car = 0; car < cars_num_; car++)
      {
          ...
          swarm_traj_container_[car].singul_traj[sur_segmentId].traj.getDirection();
          // double cal_time;
          ...
          {
              collision = true;
              cerr<<"About to collide! Replan!"<<endl;
              break;
          }
      }
